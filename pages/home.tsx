...
export const getServerSideProps = async () => {
  const data = await fetch(
    "https://api.holotools.app/v1/channels?limit=10"
  ).then((res) => res.json());
  return {
      ...
      <div className="">
        <div className="flex">
          ...
          <div className="lg:inline-flex hidden mx-4 flex-col w-[25vw] max-w-[350px]">
            <div className="bg-black bg-opacity-20 h-[50px] rounded-full w-full backdrop-blur sticky top-0 z-10 flex justify-center items-center">
              {" "}
              Search
            </div>
            <div className="sticky top-20 flex-col">
              <div className="bg-black bg-opacity-20 w-full h-[500px] rounded-md">
                <div className="flex justify-center items-start">Short Cut</div>
              </div>
            </div>
          </div>
        </div>
